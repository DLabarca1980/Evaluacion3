...
fun getUbicacion(contexto: Context, onUbicacionOK:(location: ExerciseRoute.Location) ->Unit):Unit {
    try {
...
fun AppUI(cameraController: CameraController){
        ...
        Pantalla.FORM -> {
            PantallaFormUI(
                ...
                tomarFotoClick = {
                    ...
                    cameraAppViewModel.lanzadorPermisos?.launch(arrayOf(Manifest.permission.CAMERA))   // TOMA ERROR REVISARR!!!!
                },
    ...
    Pantalla.FOTO -> {
        PantallaFotoUI(formRecepcionVM, cameraController, cameraController)
    }
    else -> run {
...
@Composable
fun PatallaFormUI(
    formRecepcionVM : FormRecepcionViewModel, tomarFotoOnClick:() -> Unit = {},
    ...
    Column (
        ...
        TextField(
            label = { Text("Ubicación")},   // DECIA RECEPTOR
            value = formRecepcionVM.ubicacion.value ,
            onValueChange = { formRecepcionVM.ubicacion.value = it },
            modifier = Modifier
                .fillMaxSize()
        ...
        formRecepcionVM.fotoUbicacion.value?.also {
            Box( Modifier.size(200.dp, 100.dp) ) {
                Image(painter = BitmapPainter(uri2imageBitmap(it, contexto)),
...
} // SEGUIR EN LA LINEA 240 ejercicioSemana6
